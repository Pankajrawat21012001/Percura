...
function ShaderPlane() {
    ...
    return (
        <mesh ref={meshRef} position={[0, 0, 0]} scale={[viewport.width * 1.1, viewport.height * 1.1, 1]}>
...
export function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="bg-black absolute inset-0 w-[110vw] -ml-[5vw] h-full" aria-hidden>
            <Canvas
                camera={camera}
...
export default function Hero({ title, description, badgeText, badgeLabel, ctaButtons, microDetails }: any) {
    ...
    useGSAP(() => {
        ...
        document.fonts.ready.then(() => {
            ...
            gsap.from(split.lines, { filter: 'blur(16px)', yPercent: 30, autoAlpha: 0, scale: 1.05, duration: 1, stagger: 0.15, ease: 'power3.out' });
        });
    }, { scope: containerRef });

    return (
        <section ref={containerRef} className="relative h-screen w-screen overflow-hidden bg-black flex flex-col justify-center">
            <ShaderBackground />
            ...
            <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
                ...
                <h1 ref={headerRef} className="text-5xl md:text-8xl font-extralight text-white leading-[1.1] mb-8">{title}</h1>
                <p className="text-xl text-white/50 font-light max-w-2xl mb-12 leading-relaxed">{description}</p>
                <div className="flex flex-wrap gap-4">
                    {ctaButtons?.map((btn: any, i: number) => (
                        <a
                            key={i}
                            href={btn.href}
                            className={`inline-flex items-center gap-3 px-8 py-4 rounded-full transition-all duration-300 font-bold ${btn.primary ? 'bg-white text-black hover:bg-white/90 scale-100 hover:scale-[1.02] active:scale-95' : 'border border-white/10 text-white/60 hover:text-white'}`}
                        >
                            {btn.text}
                            {btn.primary && (
